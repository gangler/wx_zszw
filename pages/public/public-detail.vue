<template>
	<view>
		<cu-custom bgColor="bg-gradual-blue" :isBack="true" v-if="flag === 1"><block slot="backText"></block><block slot="content">{{infoDetail.title}}</block></cu-custom>
		<cu-custom bgColor="bg-darkblue" :isBack="true" v-else-if="flag === 2"><block slot="backText"></block><block slot="content">{{infoDetail.title}}</block></cu-custom>
		<!-- <view class="box">
	...
	export default {
		...
		methods: {
			...
			getNewsInformation(val) {
				uni.request({
					url: configService.apiUrl + '/getPublicById',
					data: {
						informationid: val,
					},
					success: (res) => {
